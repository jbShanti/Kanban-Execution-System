...
| **type**              | Type of finding: Strengths, Risks, Opportunities, Observation |
| **statement**         | Natural language statement displayed in the report.        |
| **severity**          | Relative importance and urgency of the Finding: Info / Success / Warning / Critical |
...
`Evidence` provides the objective facts supporting a Finding.

Its purpose is to make every Finding traceable, reproducible, and explainable without embedding analytical logic into the Finding itself.

Evidence contains factual data only.
...
Those belong to the Finding and Recommendation models.
...
Represents a measured analytical value.
...
Identifies an object involved in the Finding.

Examples:

```yaml
- entity: task
  value: Parser Refactoring
```
...
Represents the comparison that led to the analytical conclusion.
...
Comparisons provide objective context for the Finding without interpreting the result.

---

#### Design Principles

* Evidence contains objective facts only.
* Evidence is deterministic and reproducible.
* Evidence does not contain recommendations.
* Evidence does not contain interpretations.
* A Finding may reference multiple facts.
* Facts may originate from one or more Analytics Objects.
* Evidence should contain only the minimum information required to justify the Finding.
...
Finding:
  ...
  title: Workload exceeds healthy operating range

  statement: Active workload is above the recommended operating range.
...
The Finding Catalogue defines the canonical specification for every supported Finding type generated by the Analytics Engine.

Each catalogue entry serves as a contract describing how a particular Finding is identified, classified, supported by evidence, and produced from analytical results.

The catalogue ensures that identical analytical conditions always generate Findings with the same semantic meaning and structure.

---

### Responsibilities

The Finding Catalogue is responsible for:

* defining canonical Finding types
* assigning stable Finding identifiers
* classifying Findings by semantic type
* defining supported severity levels
* specifying required and optional Analytics Objects
* defining the minimum Evidence required to support each Finding
* providing example user-facing statements

The Finding Catalogue does **not** perform analytical calculations.

The Finding Catalogue does **not** generate Findings.

The Finding Catalogue does **not** define recommendation logic or report rendering.
...
Each catalogue entry shall define:
...
| Field                 | Description                                                              |
| --------------------- | ------------------------------------------------------------------------ |
| **id**                | Stable machine-readable identifier of the Finding type.                  |
| **title**             | Human-readable name of the Finding.                                      |
| **type**              | Canonical Finding category: Observation, Strength, Risk, or Opportunity. |
| **purpose**           | Analytical purpose of the Finding.                                       |
| **severity**          | Supported severity levels that may be assigned to this Finding type.     |
| **analytics_sources** | Required and optional Analytics Objects used to generate the Finding.    |
| **minimum_evidence**  | Minimum Evidence required to justify the Finding.                        |
| **example_statement** | Example natural-language statement shown to the user.                    |

---

### Finding Catalogue Items

#### Corridor Status

##### Purpose

Describes the current state of an individual Score Corridor.
...
example_statement:
  ...
  Corridor 21–25 contains 5 active tasks and is within its target range.
```
...
Evaluates whether an individual Score Corridor operates within its target workload range.
...
example_statement:
  ...
  Corridor 6–10 exceeds its target workload.
```

---

#### System Corridor Health

##### Purpose

Evaluates the overall health of all Score Corridors.

##### Canonical Specification

```yaml
id: system_corridor_health

title: System Corridor Health
...
minimum_evidence:
  ...
  facts:
    ...
    - metric: corridor_health

example_statement:

  All five Score Corridors are simultaneously within their target ranges.
```
...
Describes the current execution workload.
...
example_statement:
  ...
  The board currently contains 19 active tasks.
```
...
Evaluates workload against the target operating range.
...
example_statement:
  ...
  The overall workload remains balanced.
```

> The remaining Finding types follow the same canonical structure:
>
> * Execution Flow
> * Execution Momentum
> * Focus Distribution
...
* Every Finding type shall have exactly one stable identifier.
* The same analytical condition shall always produce the same Finding type.
* A Finding type may support multiple severity levels.
* A Finding type may require one or more Analytics Objects.
* Optional Analytics Objects may enrich, but shall not redefine, the Finding.
* Every Finding type shall define the minimum Evidence required for generation.
* Example statements illustrate the Finding but are not normative.
...
The Finding Catalogue defines **what kinds of Findings may exist**.

The Analytics Engine instantiates those Finding types using analytical results and supporting Evidence.
